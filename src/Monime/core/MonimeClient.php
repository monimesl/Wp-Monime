 ...
 *
 * Adapters normalize platform-specific order/payment data into
 * CreatepaymentRequest, then this class handles headers, JSON encoding,
...
class MonimeClient
{
    ...
    public function create_checkout_session(CreateMonimePayload $paymentrequest, string $_handler)
    {
        ...
        //$amountValue = $paymentrequest->amount;
        ...
        $data = [
            'name' => !empty($paymentrequest->name) ? $paymentrequest->name : 'payment',
            ...
            'lineItems' => $paymentrequest->items,

            'reference' => $reference,

            'cancelUrl' => $paymentrequest->cancelurl,

            'successUrl' => $paymentrequest->success_url,

            'callbackState' => $paymentrequest->callbackState,
        ];

        // Optional request fields are included only when an adapter provides them.
        if (!empty($paymentrequest->financialAccountId)) {
            $data['financialAccountId'] = $paymentrequest->financialAccountId;
        }

        if (!empty($paymentrequest->callbackState)) {
            $data['callbackState'] = $paymentrequest->callbackState;
        }

        if (!empty($paymentrequest->paymentOptions)) {
            $data['paymentOptions'] = $paymentrequest->paymentOptions;
        }
