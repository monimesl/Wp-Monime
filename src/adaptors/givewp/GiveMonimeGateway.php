...
class GiveMonimeGateway extends PaymentGateway implements PaymentAdapterInterface, WebhookAdapterInterface
{
	...
	public function buildPaymentPayload(array $data): CreateDonationRequest
	{
		...
		return new CreateDonationRequest(
			...
			(string) ($data['financialAccountId'] ?? ''),
			(string) ($data['currency'] ?? 'SLE'),

			is_array($data['paymentOptions'] ?? null)
				? $data['paymentOptions']
				: [],
	...
	public function get_campaign_name(Donation $donation)
	{
		...
		if ($form_id) {
			...
			$campaign_name = trim(
				wp_strip_all_tags(
					give_get_donation_form_title($form_id)
				)
			);
